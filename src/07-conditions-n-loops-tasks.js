...
function doRectanglesOverlap(rect1, rect2) {
  ...
  } else if (rect1.top <= rect2.top && rect1.left > rect2.left) {
    if (rect1.top + rect1.height > rect2.top && rect2.left + rect2.width > rect1.left) {
      answer = true;
    }
  } else if (rect1.top > rect2.top && rect1.left <= rect2.left) {
    if (rect2.top + rect2.height > rect1.top && rect1.left + rect1.width > rect2.left) {
      answer = true;
    }
  } else if (rect1.top > rect2.top && rect1.left > rect2.left) {
  ...
  if (str.length > 0) {
    for (let i = 0; i < str.length; i += 1) {
      if (open.indexOf(str[i]) >= 0) {
        ...
        answer = false;
      } else if (str[i] === close[open.indexOf(arr[arr.length - 1])]) {
        arr.pop();
      } else if (str[i] !== close[open.indexOf(arr[arr.length - 1])]) {
        answer = false;
      }
...
function getCommonDirectoryPath(pathes) {
  ...
  let i = 0;
  if (pathes.length === 2) {
    while (pathes[0][i] === pathes[1][i]) {
      result += pathes[0][i];
      i += 1;
    }
  } else if (pathes.length === 3) {
    while (pathes[0][i] === pathes[1][i] && pathes[0][i] === pathes[2][i]) {
      result += pathes[0][i];
      i += 1;
    }
  }
  const lastArr = result.split('');
  if (lastArr.length > 0) {
    while (lastArr[i] !== '/') {
      lastArr.length = i;
      i -= 1;
